...
function initMusic() {
  ...
  const stopFallback = () => {
    ...
    fallbackTimer = null;
    if (fallbackContext) fallbackContext.close();
  ...
  const playFallbackNote = () => {
    ...
    oscillator.frequency.value = melody[fallbackStep % melody.length];
    gain.gain.setValueAtTime(0.0001, fallbackContext.currentTime);
    gain.gain.exponentialRampToValueAtTime(0.18, fallbackContext.currentTime + 0.02);
    gain.gain.exponentialRampToValueAtTime(0.0001, fallbackContext.currentTime + 0.42);
    oscillator.connect(gain).connect(fallbackContext.destination);
    oscillator.start();
    oscillator.stop(fallbackContext.currentTime + 0.45);
    fallbackStep += 1;
  };
  ...
  const startFallback = async () => {
    ...
    playFallbackNote();
    fallbackTimer = setInterval(playFallbackNote, 500);
  };

  audio.addEventListener("error", () => {
    missing = true;
    setState(false);
    showToast("🎵 Playing the built-in birthday tune.");
  });

  let autoStarted = false;
  const autoPlayOnInteraction = (event) => {
    const target = event.target;
    if (autoStarted || (target instanceof Element && target.closest("#music-btn"))) return;
    autoStarted = true;
    btn.click();
  };
  document.addEventListener("pointerdown", autoPlayOnInteraction, { passive: true });
  document.addEventListener("keydown", autoPlayOnInteraction);

  btn.addEventListener("click", async () => {
    if (missing && fallbackTimer) {
      stopFallback();
      setState(false);
      return;
    }
    if (missing) {
      try {
        ...
        stopFallback();
        setState(false);
        showToast("Couldn't play the birthday music. Tap again to retry ♪");
      }
      return;
    }
    if (audio.paused) {
